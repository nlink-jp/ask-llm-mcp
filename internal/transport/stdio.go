// Package transport provides the stdio JSON-RPC transport used by the
// MCP server.
//
// Each JSON-RPC message is one line (newline-terminated). The buffer is
// sized to 1MB per line, matching the pattern in mcp-guardian and
// data-toolbox-mcp.
package transport
...
// ReadMessage returns the next line from stdin, or io.EOF when stdin is
// closed. The returned slice is owned by the caller (a fresh copy).
...
// WriteMessage encodes msg as JSON and writes it as a single
// newline-terminated line. Writes are serialized; safe to call from
// multiple goroutines.
